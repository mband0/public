...
def lambda_handler(event,context):
    CONGRESS_URL = 'https://api.congress.gov/v3'
    URL_ENDPOINT = '/committee'
    table_name = "committees"
    columns_list = ['chamber','committeeTypeCode','parent','name','subcommittees','systemCode','url']
    columns = "(chamber,committeecode,parent,name,subcommittees,systemcode,url)"
    limit=250
    
    params = f"?api_key={API_KEY}&format=json&limit={limit}"
    request_url = CONGRESS_URL + URL_ENDPOINT + params
    
    total_data = []
    while request_url:
        response_data,request_url = get_response(request_url)
        if response_data:
            data = response_data["committees"]
            total_data += data
