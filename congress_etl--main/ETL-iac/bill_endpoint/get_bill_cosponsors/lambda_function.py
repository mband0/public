"""Lambda function that calls Bill Cosponsors API and writes data to a db."""
import os
import json
import requests
import boto3
...
def lambda_handler(event,context):
    ...
    URL_ENDPOINT = '/cosponsors'
    table_name = "bill_cosponsors"
    columns_list = ["bioguideId","firstName","fullName","isOriginalCosponsor","lastName","party","sponsorshipDate","state","url","congress", "billType", "billNumber", "endpoint_url"]
    columns = "(bioguideId,firstName,fullName,isOriginalCosponsor,lastName,party,sponsorshipDate,state,url,congress,billType,billNumber,endpoint_url)"
    limit = 250
    ...
    for row_data in input_data:
        ...
        while request_url:
            ...
            if response_data:
                cosponsors_data = response_data["cosponsors"]
                for item in cosponsors_data:
