"""Lambda function that calls Bill info API and writes the data to a db."""
import os
import json
import requests
import boto3
...
def lambda_handler(event,context):
    ...
    Arguments:
    event: file name from s3.
    context: Not in use (mandatory for lambda function)
    """
    CONGRESS_URL = 'https://api.congress.gov/v3'
    URL_ENDPOINT = ''
    table_name = "bill_info"
    columns_list = ["actions","amendments","committees","congress","cosponsors","introduceddate","latestaction","number","originchamber","originchambercode","policyarea","relatedbills","sponsors","subjects","summaries","textversions","title","titles","type","updatedate","updatedateincludingtext","endpoint_url"]
    columns = "(actions,amendments,committees,congress,cosponsors,introduceddate,latestaction,number,originchamber,originchambercode,policyarea,relatedbills,sponsors,subjects,summaries,textversions,title,titles,type,updatedate,updatedateincludingtext,endpoint_url)"
    limit = 250
    bill_data_file = event.get("Key", "No file provided")
    if bill_data_file=="No file provided":
        return {"status": 404,
            "file": bill_data_file}
    ...
    total_data = []
    bills_data = read_file(s3,BUCKET,bill_data_file)
    for bill_data in bills_data:
        ...
        if response_data:
            bill_info_data = response_data["bill"]
            for item in bill_info_data:
                item.update( {"endpoint_url":request_url})
            total_data.append(bill_info_data)
