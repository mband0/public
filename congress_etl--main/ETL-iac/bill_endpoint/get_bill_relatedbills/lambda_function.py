"""Lambda function that calls Bill Relatedbills API and writes the data to a db."""
import os
import json
import requests
import boto3
...
def lambda_handler(event,context):
    ...
    URL_ENDPOINT = '/relatedbills'
    table_name = "bill_relatedbills"
    columns_list = ["congress","latestAction","number","relationshipDetails","title","type","url","endpoint_congress","endpoint_billType","endpoint_billNumber","endpoint_url"]
    columns = "(congress,latestAction,number,relationshipDetails,title,type,url,endpoint_congress,endpoint_billType,endpoint_billNumber,endpoint_url)"
    limit = 250
    ...
    for row_data in input_data:
        ...
        while request_url:
            ...
            if response_data:
                relatedbills_data = response_data["relatedBills"]
                for item in relatedbills_data:
                    item.update( {"endpoint_congress":congress})
                    item.update( {"endpoint_billType":bill_type})
                    item.update( {"endpoint_billNumber":bill_number})
                    item.update( {"endpoint_url":request_url})
                total_data += relatedbills_data
