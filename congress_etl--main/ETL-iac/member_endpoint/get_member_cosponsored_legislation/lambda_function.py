...
def lambda_handler(event,context):
    ...
    URL_ENDPOINT = '/cosponsored-legislation'
    table_name = "member_cosponsored_legislation"
    columns_list = ['congress','introducedDate','latestAction','number','policyArea','title','type','url']
    columns = "(congress,introduceddate,latestaction,number,policyarea,title,type,url)"
    limit=250
    ...
    for data in input_data:
        ...
        while request_url:
            ...
            if response_data:
                actions_data = response_data["cosponsoredLegislation"]
                for item in actions_data:
                    item.update( {"member_id":member_id})
